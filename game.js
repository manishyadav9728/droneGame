...
let flyDrone = PIXI.Sprite.from('assets/drone.png');
...
tl.add([gsap.to(flyDrone,{x:550,duration:5}),gsap.to(flyDrone,{rotation:east})])

tl.add([gsap.to(flyDrone,{y:550,duration:5}),gsap.to(flyDrone,{rotation:south})]);
tl.add([gsap.to(flyDrone,{x:stepSize,duration:5}),gsap.to(flyDrone,{rotation:west})]);
tl.add([gsap.to(flyDrone,{y:stepSize,duration:5}),gsap.to(flyDrone,{rotation:north})]);


 function placeFunction(){
    if(xPos.value<0 || xPos.value >10 || yPos.value<0 || yPos.value>10 || direction.indexOf(face.value)<=-1 || xPos.value == 0 || yPos.value == 0 || face.value ==0){
      window.alert("please enter  value in format:X and Y's  values in range 0<value<10 and  face in capital letter Ex: S,W,E,N");
    }else{
      xPos.value *= stepSize; 
      yPos.value *= stepSize;
      tl.pause();
 ...
 function reportFunction(){
  window.alert(`Drone is at position X:${Math.round(flyDrone.x/50)},Y:${Math.round(flyDrone.y/50)} and face:${face.value}`);
 }
